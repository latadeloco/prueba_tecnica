...
require_once BASE_DIR . "/ej1/app/Classes/FibonacciDateNumber.php";

class Fibonacci implements \Interfaces\OutputInterface
{
    private $fibonacci, $result;
    private Timestamp $initialTimestamp, $endTimestamp;

    public function __construct(Timestamp $initialTimestamp, Timestamp $endTimestamp)
    {
        try {
    ...
    { 
        ...
        $initYear = new Timestamp( strtotime( $this->initialTimestamp->format('Y') . '-01-01 00:00:00' ) );
        ...
        $secondDay = new Timestamp( strtotime( $initYear->format('Y') . '-01-02 00:00:00' ) );
        ...
        $nextTimestamp = new Timestamp(strtotime( $this->fibonacci[ count($this->fibonacci) - 1 ]->getDate() ) );
    ...
    private function preCompareTimestamps(Timestamp $init, Timestamp $end)
    {
    ...
    public function outputAPI()
    {
        if (isset($this->result)) {
            ...
            return json_encode($arryToJsonResult);
        }
    }
}
